...
void main() {
  ...
  vec2 displacement = vec2(
    dot(flow.rg, vec2(1.0, 1.0 / 255.0)),
    dot(flow.ba, vec2(1.0, 1.0 / 255.0))
  ) * 2.0 - 1.0;
  uv -= displacement * vec2(aspect, 1.0);
...
// Store horizontal and vertical displacement at 16-bit precision in an RGBA8 texture.
const flowFragmentShader = `#version 300 es
...
vec2 displacementAt(sampler2D field, vec2 uv) {
  vec4 value = texture(field, uv);
  return vec2(
    dot(value.rg, vec2(1.0, 1.0 / 255.0)),
    dot(value.ba, vec2(1.0, 1.0 / 255.0))
  ) * 2.0 - 1.0;
}
vec2 encode(float value) {
  vec2 packed = fract((clamp(value, -0.99, 0.99) * 0.5 + 0.5) * vec2(1.0, 255.0));
  packed.x -= packed.y / 255.0;
  return packed;
}
void main() {
  if (uReset == 1) {
    fragColor = vec4(encode(0.0), encode(0.0));
    return;
  }
  vec2 current = displacementAt(uFlow, vUv);
  vec2 velocity = current - displacementAt(uPreviousFlow, vUv);
  vec2 neighbors = displacementAt(uFlow, vUv + vec2(uTexel.x, 0.0))
    + displacementAt(uFlow, vUv - vec2(uTexel.x, 0.0))
    + displacementAt(uFlow, vUv + vec2(0.0, uTexel.y))
    + displacementAt(uFlow, vUv - vec2(0.0, uTexel.y));
  // Retain momentum, spread it locally, and gently spring back toward rest.
  vec2 next = current + velocity * 0.94 + (neighbors - current * 4.0) * 0.15 - current * 0.008;
  vec2 aspect = vec2(uAspect, 1.0);
  vec2 offset = (vUv - uPointer.xy) * aspect;
  vec2 direction = (uPointer.zw - uPointer.xy) * aspect;
  float along = clamp(dot(offset, direction) / max(dot(direction, direction), 0.000001), 0.0, 1.0);
  float distance = length(offset - along * direction);
  float brush = exp(-pow(distance / 0.14, 2.0));
  vec2 drag = direction * min(1.0, 0.08 / max(length(direction), 0.000001));
  next += drag / aspect * brush * 0.12;
  float edge = min(min(vUv.x, 1.0 - vUv.x), min(vUv.y, 1.0 - vUv.y));
  next *= smoothstep(0.0, 0.04, edge);
  fragColor = vec4(encode(next.x), encode(next.y));
}`
...
export default function Matrix({
  ...
  const palette = useMemo(
    () =>
      buildPaletteUniforms(Array.isArray(colors) && colors.length > 0 ? colors : DEFAULT_COLORS),
    [colors],
  )
  ...
  const rootStyle = useMemo(() => ({ background: back, ...style }), [back, style])
  ...
  useEffect(() => {
    ...
    const flowHeight = 192
    const flowWidth = Math.max(
      1,
      Math.round((flowHeight * container.clientWidth) / Math.max(container.clientHeight, 1)),
    )
    const flowTargets = Array.from(
      { length: 3 },
      () =>
        new OglRenderTarget(gl, {
          width: flowWidth,
          height: flowHeight,
          depth: false,
          minFilter: gl.LINEAR,
    ...
    let [flowPrevious, flowRead, flowWrite] = flowTargets
    const pointer = [0, 0, 0, 0]
    const flowTexel = [1 / flowWidth, 1 / flowHeight]
    const flowProgram = new Program(gl, {
      vertex: dotVertexShader,
      fragment: flowFragmentShader,
      depthTest: false,
      depthWrite: false,
      uniforms: {
        uFlow: { value: flowRead.texture },
        uPreviousFlow: { value: flowPrevious.texture },
        uTexel: { value: flowTexel },
        uPointer: { value: pointer },
        uAspect: { value: flowWidth / flowHeight },
        uReset: { value: 1 },
      },
    })
    ...
    for (let i = 0; i < flowTargets.length; i++) {
      flowProgram.uniforms.uFlow.value = flowRead.texture
      flowProgram.uniforms.uPreviousFlow.value = flowPrevious.texture
      renderer.render({ scene: flowMesh, target: flowWrite })
      ;[flowPrevious, flowRead, flowWrite] = [flowRead, flowWrite, flowPrevious]
    }
    flowProgram.uniforms.uReset.value = 0
    ...
    const update = (time: number) => {
      ...
      flowProgram.uniforms.uAspect.value = gl.canvas.width / gl.canvas.height
      flowProgram.uniforms.uFlow.value = flowRead.texture
      flowProgram.uniforms.uPreviousFlow.value = flowPrevious.texture
      renderer.render({ scene: flowMesh, target: flowWrite })
      ;[flowPrevious, flowRead, flowWrite] = [flowRead, flowWrite, flowPrevious]
      perlinProgram.uniforms.uFlow.value = flowRead.texture
      pointer[0] = pointer[2]
      pointer[1] = pointer[3]
      renderer.render({
        scene: perlinMesh,
  ...
  useEffect(() => {
    ...
    if (!isPlayingRef.current) renderOnce()
  }, [effectivePlay, frequency, speed, effPaletteCount, palette, cellSize, gamma, paletteBias])
  ...
  useEffect(() => {
    ...
    if (!useGlyphAtlasFlag || !gl) {
      dot.uniforms.uUseGlyphAtlas.value = 0
      dot.uniforms.uCharCount.value = 0
      if (dummyGlyphTextureRef.current) {
        dot.uniforms.uGlyphAtlas.value = dummyGlyphTextureRef.current
      }
    } else {
      try {
        const f = deriveFontSettings(fontFamily, fontWeight, fontSizePx)
    ...
    if (!isPlayingRef.current) renderOnce()
  }, [useGlyphAtlasFlag, effectiveCharacters, fontFamily, fontWeight, fontSizePx])

  return (
    <div
      className={`relative h-full min-h-0 w-full min-w-0 overflow-hidden leading-0 ${className}`}
      style={rootStyle}
    >
      <div className="pointer-events-none invisible absolute h-100 min-h-100 w-150 min-w-150" />
