    ...
    <div className="min-h-screen">
      <section className="relative flex min-h-screen items-end overflow-hidden px-6 pt-36 pb-20 md:pb-24">
        <div className="absolute inset-x-6 top-28 z-20">
          <div className="mx-auto flex max-w-6xl justify-end">
            <Link
              ...
              className={`group inline-flex items-center gap-2 border-b border-border/70 pb-2 font-eyebrow text-[10px] tracking-[0.2em] text-muted-foreground uppercase opacity-0 transition-colors [animation-delay:220ms] hover:text-foreground ${introVisible ? "animate-fade-in-up" : ""}`}
            >
        ...
        <div className="mx-auto grid w-full max-w-6xl items-end gap-16 lg:grid-cols-[minmax(0,1.35fr)_minmax(18rem,0.65fr)]">
          <div>
            <p
              className={`mb-9 font-eyebrow text-xs font-medium tracking-[0.3em] text-muted-foreground uppercase opacity-0 ${introVisible ? "animate-fade-in-up" : ""}`}
            >
              {config.eyebrow}
            </p>
            <h1 className="max-w-5xl font-display text-[clamp(2.35rem,5.85vw,5.2rem)] leading-[0.9] tracking-[-0.045em] text-foreground">
              <span
          ...
          >
            <p className="mb-8 text-lg leading-relaxed text-muted-foreground">{config.intro}</p>
      ...
      <section className="border-y border-border/40 bg-white/45 px-6 py-24 backdrop-blur-sm">
        <div className="mx-auto grid max-w-6xl gap-14 lg:grid-cols-[0.85fr_1.15fr] lg:gap-24">
          <Reveal>
            <p className="mb-5 font-eyebrow text-xs font-medium tracking-[0.28em] text-muted-foreground uppercase">
              Built for more
            </p>
            <h2 className="font-display text-4xl leading-tight font-semibold text-foreground md:text-5xl">
              {config.promise}
            </h2>
          </Reveal>
          ...
          <Reveal className="mx-auto max-w-6xl overflow-hidden rounded-3xl border border-border/50 bg-black text-white">
            <div className="grid lg:grid-cols-[1.1fr_0.9fr]">
              <div className="p-9 md:p-14">
                <p className="mb-6 font-eyebrow text-xs tracking-[0.28em] text-white/50 uppercase">
                  {config.companion.eyebrow}
                </p>
                <h2 className="mb-6 max-w-xl font-display text-4xl leading-tight md:text-5xl">
                  {config.companion.title}
                </h2>
                <p className="mb-9 max-w-xl leading-relaxed text-white/65">
                  {config.companion.description}
                </p>
                <Link
      ...
      <section id="service-process" className="px-6 py-28 md:py-36">
        <div className="mx-auto max-w-6xl">
          <Reveal className="mb-20 max-w-3xl">
            ...
            <h2 className="mb-7 font-display text-4xl leading-tight font-semibold text-foreground md:text-6xl">
              {config.processTitle}
            </h2>
            ...
            <div className="hidden lg:block">
              <div className="sticky top-28">
                <div className="mb-7 flex items-end justify-between">
                  <p className="font-eyebrow text-xs tracking-[0.25em] text-muted-foreground uppercase">
                    Active phase
                  </p>
                ...
                <div className="mt-7 grid gap-2">
                  {config.steps.map((step, index) => (
                    ...
                    >
                      <span className="w-5 font-mono text-[10px] tabular-nums">{step.number}</span>
                ...
                >
                  <div className="w-full">
                    <div className="mb-9 flex items-center justify-between gap-4">
                      ...
                      <span className="font-eyebrow text-[10px] tracking-[0.24em] text-muted-foreground uppercase">
                        {step.label}
                      </span>
                    </div>
                    ...
                    <p className="mb-9 max-w-2xl text-lg leading-relaxed text-muted-foreground">
                      {step.description}
                    </p>
      ...
      >
        <div className="mx-auto max-w-6xl">
          <Reveal className="mb-12 flex flex-col justify-between gap-6 md:flex-row md:items-end">
            <div>
              <p className="mb-4 font-eyebrow text-xs tracking-[0.28em] text-muted-foreground uppercase">
                {config.systemsLabel}
              </p>
      ...
      <section className="px-6 py-28">
        <Reveal className="mx-auto max-w-5xl rounded-3xl border border-border/50 bg-white/65 p-10 text-center shadow-sm backdrop-blur-xl md:p-20">
          <p className="mb-5 font-eyebrow text-xs tracking-[0.28em] text-muted-foreground uppercase">
